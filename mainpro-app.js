...
(() => {
      ...
      const cal = new FC.Calendar(el,{
        ...
        datesSet:(info)=>{
          ...
          try{
            ...
            // Persist last opened date (YYYY-MM-DD)
            try{
              localStorage.setItem('mainpro_lastdate_v1', center.toISOString().slice(0,10));
            }catch{}
  ...
  function generateSeries(base, freq, months){
    ...
    if(freq==='weekly'){
      ...
      const interval = Math.max(1, Number(base.recurOptions?.interval) || 1);
      const unit = base.recurOptions?.unit || 'day';
      while(true){
        let next;
        if(unit==='day') next = addDays(baseStart, interval*i);
        else if(unit==='week') next = addDays(baseStart, interval*7*i);
        else if(unit==='month') next = addMonths(baseStart, interval*i);
        else if(unit==='year') next = addMonths(baseStart, 12*interval*i);
        else next = addDays(baseStart, interval*i);
      ...
      if(out.length === 0){
        let cursor = addDays(baseStart, 14);
        while(cursor <= endLimit){
          const clone={...base, id: `${base.seriesId}-${i}`, start: toLocalISO(cursor), isInstance:true};
          out.push(clone);
          i++;
      ...
      const dom = Number(base.recurOptions?.monthDay) || baseDom;
      while(true){
        const next = addMonths(baseStart, 6*i);
        next.setHours(baseStart.getHours(), baseStart.getMinutes(), 0, 0);
        const target = new Date(next);
        target.setDate(dom);
        if(target > endLimit) break;
    ...
    } else if(freq==='every4months'){
      const baseDom = baseStart.getDate();
      const dom = Number(base.recurOptions?.monthDay) || baseDom;
      while(true){
        const next = addMonths(baseStart, 4*i);
        next.setHours(baseStart.getHours(), baseStart.getMinutes(), 0, 0);
        const target = new Date(next);
        target.setDate(dom);
        if(target > endLimit) break;
    ...
    } else if(freq==='every9months'){
      const baseDom = baseStart.getDate();
      const dom = Number(base.recurOptions?.monthDay) || baseDom;
      while(true){
        const next = addMonths(baseStart, 9*i);
        next.setHours(baseStart.getHours(), baseStart.getMinutes(), 0, 0);
        const target = new Date(next);
        target.setDate(dom);
        if(target > endLimit) break;
    ...
    } else if(freq==='every18months'){
      const baseDom = baseStart.getDate();
      const dom = Number(base.recurOptions?.monthDay) || baseDom;
      while(true){
        const next = addMonths(baseStart, 18*i);
        next.setHours(baseStart.getHours(), baseStart.getMinutes(), 0, 0);
        const target = new Date(next);
        target.setDate(dom);
        if(target > endLimit) break;
    ...
    } else if(freq==='every2years'){
      const baseDom = baseStart.getDate();
      const dom = Number(base.recurOptions?.monthDay) || baseDom;
      while(true){
        const next = addMonths(baseStart, 24*i);
        next.setHours(baseStart.getHours(), baseStart.getMinutes(), 0, 0);
        const target = new Date(next);
        target.setDate(dom);
        if(target > endLimit) break;
    ...
    } else if(freq==='every3years'){
      const baseDom = baseStart.getDate();
      const dom = Number(base.recurOptions?.monthDay) || baseDom;
      while(true){
        const next = addMonths(baseStart, 36*i);
        next.setHours(baseStart.getHours(), baseStart.getMinutes(), 0, 0);
        const target = new Date(next);
        target.setDate(dom);
        if(target > endLimit) break;
      ...
      while(true){
        let next;
        if(freq==='quarterly') next = addMonths(baseStart, 3*i);
        else if(freq==='yearly')    next = addMonths(baseStart, 12*i);
        else next = addMonths(baseStart, 1*i);
        if(next > endLimit) break;
        const clone={...base, id: `${base.seriesId}-${i}`, start: toLocalISO(next), isInstance:true};
        out.push(clone);
        i++;
        ...
        if(cal){
          ...
          toAdd.forEach(e=>{
            ...
            cal.addEvent({
              ...
              id:e.id, title:e.title.length > 12 ? e.title.substring(0, 12) + '...' : e.title, start:s, allDay:false,
    ...
    return React.createElement('div', {className:`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-[#FDFCF8]'}`, style:{'--acc': ui.primary}},
      ...
      React.createElement('div', {className:"glassbar"},
        ...
        React.createElement('div',{className:"max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between"},
          ...
          React.createElement('div',{className:"flex items-center gap-3"},
            ...
            React.createElement('button',{onClick:(e)=>{ e.stopPropagation(); if(!openSettings) setOpenSettings(true); }, className:"px-3 py-2 rounded-lg bg-white border hover:bg-gray-100 shadow-sm"},'⚙️ Settings'),
            
            // 💬 AI Chat Button (simple fallback — React modal may not open)
            React.createElement('button',{
              onClick:()=>{ if (window.openSimpleAIChatModal) window.openSimpleAIChatModal(); else setShowAIChat(true); },
            ...
            !isAuthenticated ? 
              React.createElement('button',{
                onClick:()=>{ if (window.openSimpleAuthModal) window.openSimpleAuthModal(); else { setAuthMode('login'); setShowAuthModal(true); } },
      ...
      openSettings && React.createElement('div',{className:"fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4 mp-overlay-anim", style:{overflow:'hidden'}, 'data-mp-overlay':'1', onClick:(e)=>{ if(e.target===e.currentTarget) mpCloseWithAnim(()=>setOpenSettings(false), e); }},
              ...
              React.createElement('div',{className:"p-4 border-2 rounded-xl bg-amber-50 shadow-md border-amber-200"},
                ...
                React.createElement('div',{className:"flex flex-col sm:flex-row gap-2"},
                  React.createElement('button',{
                    onClick:()=>{
                      try{
                        const fileName = `MainPro_Backup_${currentCalendarId}_${new Date().toISOString().slice(0,19).replace(/[:T]/g,'-')}.json`;
                        const payload = {
                          version: 1,
                  ...
                  React.createElement('button',{
                    onClick:()=>{
                      try{
                        const fileName = `MainPro_Backup_ALL_${new Date().toISOString().slice(0,19).replace(/[:T]/g,'-')}.json`;
                        const cals = Array.isArray(calendars) ? calendars : [];
                        const eventsByCalendar = {};
                        cals.forEach(c=>{
                          try{
                            const key = `mainpro_calendar_${c.id}`;
                            eventsByCalendar[c.id] = safeParse(key, []);
      ...
      showAuthModal && React.createElement('div',{className:"fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 mp-overlay-anim", 'data-mp-overlay':'1', onClick:(e)=>{ if(e.target===e.currentTarget) mpCloseWithAnim(()=>setShowAuthModal(false), e); }},
      ...
      showAIChat && React.createElement('div',{className:"fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 mp-overlay-anim", 'data-mp-overlay':'1', onClick:(e)=>{ if(e.target===e.currentTarget) mpCloseWithAnim(()=>setShowAIChat(false), e); }},
  ...
  // Safe localStorage parser utility (already defined above)

  // === MAINPRO v71 - CLOUD & COLLABORATION FUNCTIONS ===
  
  // Save user profile to localStorage
  const saveUserProfile = (userData) => {
    Object.keys(userData).forEach(key => {
      localStorage.setItem(`mainpro_user_${key}`, userData[key]);
    });
  ...
  const inviteTeamMember = async () => {
    ...
    localStorage.setItem('mainpro_team_members', JSON.stringify(updatedMembers));
    ...
    // Log the invitation
    logAuditEvent('team_invite', `Invited ${inviteEmail} as ${inviteRole}`, currentUser.name);
  };
  ...
  const removeTeamMember = (memberId) => {
    ...
    logAuditEvent('team_remove', 'Removed team member', currentUser.name);
  };
  ...
  const updateMemberRole = (memberId, newRole) => {
    ...
    logAuditEvent('role_update', `Updated role to ${newRole}`, currentUser.name);
  };

  // Simulate real-time updates (in real implementation, this would use WebSocket)
  const simulateRealtimeUpdate = (type, data) => {
    const update = {
      id: Date.now(),
      type,
      data,
    ...
    logAuditEvent('realtime_update', `${type}: ${data}`, currentUser.name);
  };
  ...
  console.log("🧩 MainPro render started");
